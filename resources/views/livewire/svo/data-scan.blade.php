<div>
    ...
    <input type="password" wire:model.live="secret_for_scan" class="w-[100px]"
                    ...
                    @if( !empty( $listFiles ) )
                        <div class="w-full border border-1" style="max-height: 200px; overflow: auto;">
                            @foreach( $listFiles as $f )
                                <a href="/storage/images/{{ $f }}" target="_blank"
                                   class="text-blue-400 underline">{{ $f }}</a>
                                <br/>
                            @endforeach
                        </div>
                    @else
    ...
    ">
                <h3 class="bg-orange-400 p-2">Файлы на сервере которые не используются на сайте ({{ $sizeNoFiles }}
                    &nbsp;Mb)</h3>
    ...
    bg-gradient-to-br from-red-200 to-white
    ">
                <h3 class="bg-red-400 p-2">Файл доступа к фин части</h3>
                <div class="p-2">
                    <livewire:svo.data-scan.fin-file />
                </div>
            </div>
